...
type value =
  | IVal of int
  | NVal of string
  | CVal of string*string*cmds*env
  | UVal
...
let rec lookup (env: env) name =
  match (env) with
  | (x,y)::env -> if x = name then Some y else lookup env name
  | [] -> None
...
let rec interp st cmds log env =
  ...
  | Let :: cmds -> (
      match st with
      | x :: NVal m :: st -> interp st cmds log ((m,x)::env)
      | _ -> (Error, log)) 
  | Lookup :: cmds -> (
      match st with
      | NVal y :: st ->(
          match(lookup env (y)) with
          |Some x -> interp (x::st) cmds log env
          |None -> (Error,log))
      | _ -> (Error, log))
  | Begin (cmds1):: cmds ->(
      match interp [] cmds1 log env with
      | (Ok (x::_),log) -> interp (x::st) cmds log env
      | _ -> (Error, log))
  | Fun(f, a, c) :: cmds -> interp st cmds log((f,CVal (f,a,c,env))::env)

  | Call :: cmds -> (
      match st with
      | n :: (CVal(f,arg,c,env2)):: st2 ->
        (match interp [] c log ((arg,n)::(f, CVal (f, arg, c, env2) )::env2) with
         | Ok (n1::x), log -> interp (n1::st2) cmds log env
         | _ -> (Error,log) )
      | _ -> (Error,log) )
